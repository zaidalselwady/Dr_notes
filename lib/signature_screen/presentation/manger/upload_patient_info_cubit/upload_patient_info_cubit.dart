...
class UploadPatientInfoCubit extends Cubit<UploadPatientInfoState> {
  ...
  Future<void> uploadPatientInfo(PatientInfo childInfo) async {
    ...
    var response = await dataRepo.fetchWithSoapRequest("SProc_cmd",
        "exec dbo.SProc_Patients_Info '${childInfo.name}' , '${childInfo.birthDate}' , '${childInfo.address}' , '${childInfo.phone}' , '${childInfo.email}' , '${childInfo.school}' , '${childInfo.motherName}'");
    ...
    response.fold((failure) {
      ...
      final resultElement =
          document.findAllElements('SProc_cmdResult').first;
